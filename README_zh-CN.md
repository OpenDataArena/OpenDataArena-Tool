...
1. **ODA Leaderboard** 
  ...
  * **统一基准测试**：评估训练后模型在多个领域（通用、数学、代码、科学及长链推理）的表现。
  * **标准化环境**：通过使用固定的模型规模（Llama3 / Qwen2 / Qwen3 7-8B）和一致的训练配置来严格控制变量。
  * **多模态扩展**：现已支持多模态数据集的质量评估，利用最先进的 Qwen3-VL 作为基座模型，提供真实的训练反馈。
...
3. **多维数据评分**
  ...
  * **丰富的指标库**：近期已扩展至支持 80 多个评分维度，用户只需一键即可生成全面的数据质量报告。
...
ODA 已覆盖 **4+ 个领域**、**20+ 个基准测试**、**60+ 个评分维度**，处理了 **120+ 个数据集**，评估了 **4000 万+ 个样本**，完成了 **600+ 次训练运行** 和 **1 万+ 次评测** —— 各项指标仍在持续增长中。

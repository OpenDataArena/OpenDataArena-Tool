...
**Alternative Models**: While `MingZhong/unieval-fact` is the default and recommended model for factual consistency evaluation, you can specify any compatible UniEval fact-checking checkpoint or locally fine-tuned model by providing its path in the `model` configuration parameter.

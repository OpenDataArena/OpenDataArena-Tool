...
**Alternative Models**: While `MingZhong/unieval-dialog` is the default and recommended model for dialogue evaluation, you can specify any compatible UniEval dialogue checkpoint or locally fine-tuned model by providing its path in the `model` configuration parameter.

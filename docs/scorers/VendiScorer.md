...
The Vendi Score **does not require a specific underlying model**. Instead, it operates on **pre-computed embeddings** that should be generated before running the scorer. Users can choose any appropriate embedding model for their specific domain:

* **For text data**: Sentence transformers (e.g., `sentence-transformers/all-MiniLM-L6-v2`), language model embeddings (e.g., BERT, RoBERTa)
* **For image data**: Vision models (e.g., CLIP, ResNet, Inception)
* **For other domains**: Domain-specific encoders or feature extractors

The choice of embedding model defines what notion of similarity (and thus diversity) is being measured.

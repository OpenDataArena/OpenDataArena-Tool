...
This scorer does **not require a specific language model**. Instead, it operates on **pre-computed embeddings** that can be obtained from any embedding model of your choice (e.g., sentence transformers, instruction-tuned language models, or domain-specific encoders). The embeddings should capture the semantic or structural properties of the instruction tuning data.

**Embedding Preparation:**
* Extract embeddings for each data sample using your preferred embedding model
* Save embeddings as a NumPy array (`.npy` file) with shape `[num_samples, embedding_dim]`
* Ensure the order of embeddings matches the order of samples in your dataset file

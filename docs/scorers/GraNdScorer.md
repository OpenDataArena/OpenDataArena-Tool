...
The scorer uses causal language models (CLMs) to compute gradient norms. By default, it uses [Qwen/Qwen3-8B](https://huggingface.co/Qwen/Qwen3-8B), but can work with any compatible language model including:

- Local model checkpoints (specified by file path)
- HuggingFace model identifiers (e.g., `meta-llama/Llama-2-7b-hf`, `gpt2`)

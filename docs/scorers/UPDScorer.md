...
| `model` | string | **Required** | HuggingFace model path for the causal language model used to compute probabilities. Can be any autoregressive LM (e.g., LLaMA, GPT-2, Qwen, Mistral). |
...
The scorer uses causal language models from the HuggingFace ecosystem to compute token-level probabilities and probability distributions. By default, it uses **meta-llama/Llama-2-7b-hf**, but can be configured to use any autoregressive language model such as LLaMA, GPT, Qwen, or Mistral series. The model computes the probability distribution over the vocabulary at each token position, enabling the calculation of cross-entropy loss and Shannon entropy for UPD scoring.

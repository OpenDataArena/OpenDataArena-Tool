...
def _compute_cluster_inertia(args):
    """Compute inertia for a single cluster
    
    Args:
        args: Tuple of (cluster_id, cluster_points, centroid, distance_metric)
    
    Returns:
        Tuple of (cluster_id, cluster_inertia, cluster_size)
    """
    cluster_id, cluster_points, centroid, distance_metric = args
    
    cluster_size = len(cluster_points)
    if cluster_size == 0:
        return (cluster_id, 0.0, 0)
    ...
    return (cluster_id, cluster_inertia, cluster_size)
...
class ClusterInertiaScorer(BaseScorer):
    ...
    def evaluate(self, dataset) -> Dict:
        ...
        for cluster_id in range(self.num_clusters):
            ...
            cluster_tasks.append((cluster_id, cluster_points, centroid, self.distance_metric))
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            results = list(tqdm(
                executor.map(_compute_cluster_inertia, cluster_tasks),
                total=self.num_clusters,
                desc="Computing cluster inertias"
            ))
            
            # Aggregate results
            for cluster_id, cluster_inertia, cluster_size in results:

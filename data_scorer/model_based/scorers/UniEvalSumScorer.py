...
from .UniEval_metric.evaluator import SumEvaluator
from .UniEval_metric import convert_to_json

from .base_scorer import BaseScorer
from .utils import get_total_lines


class UniEvalSumScorer(BaseScorer):
    """UniEval-based Text Summarization Evaluator
    ...
    def _validate_config(self):
        ...
        if "dimensions" not in self.config:
            self.config["dimensions"] = ['coherence', 'consistency', 'fluency', 'relevance']
            print("Warning: No dimensions specified. Using default: ['coherence', 'consistency', 'fluency', 'relevance']")
        else:
            valid_dims = ['coherence', 'consistency', 'fluency', 'relevance']
    ...
    def _setup(self):
        ...
        if is_local:
            try:
                ...
                print("Setting up UniEvalSumScorer successfully with local model")
                return
        ...
        try:
            ...
            print("Setting up UniEvalSumScorer successfully")
        except Exception as e:
            raise RuntimeError(f"Failed to initialize UniEval SumEvaluator: {e}") from e
    ...
    def _extract_data(self, data_item: Dict) -> Dict:
        ...
        # Reference is required and must not be empty
        reference = data_item["reference"]
        if not reference:
            raise ValueError(f"'reference' field is required and cannot be empty for data item with id: {data_item.get('id', 'unknown')}")
    ...
    def score_item(self, data_item: Dict) -> Dict[str, float]:
        ...
        full_text = extracted["source"] + extracted["output"] + extracted["reference"]
    ...
    def evaluate(self, dataset: str) -> List[Dict]:
        ...
        with open(dataset, 'r', encoding='utf-8') as f:
            pbar = tqdm(total=num_lines, desc=self.config.get('name', 'UniEvalSumScorer'))
            try:
                for line in f:
        ...
        for i in tqdm(range(0, len(all_data), batch_size), desc="Evaluating batches"):
            ...
            for j, score_dict in enumerate(eval_scores):
                ...
                for dim, score in score_dict.items():
                    result[f"UniEval_Sum_{dim.capitalize()}"] = score

...
from .UniEval_metric.evaluator import DialogEvaluator
...
class UniEvalDialogScorer(BaseScorer):
    """UniEval-based dialogue generation evaluator
    ...
    def _validate_config(self):
        if "model" not in self.config:
            print("Warning: No model specified. Using default: MingZhong/unieval-dialog")
            self.config['model'] = 'MingZhong/unieval-dialog'
        else:
            print(f"Using specified model: {self.config['model']}")
        
        # Validate dimensions
        if "dimensions" not in self.config:
            self.config["dimensions"] = ['naturalness', 'coherence', 'engagingness', 'groundedness', 'understandability']
            print("Warning: No dimensions specified. Using default dimensions.")
        else:
            valid_dims = ['naturalness', 'coherence', 'engagingness', 'groundedness', 'understandability']
    ...
    def _setup(self):
        ...
        model_path = self.config.get('model', 'MingZhong/unieval-dialog')
        ...
        if is_local:
            try:
                ...
                print("Setting up UniEvalDialogScorer successfully with local model")
                return
        ...
        try:
            ...
            print("Setting up UniEvalDialogScorer successfully")
        except Exception as e:
            raise RuntimeError(f"Failed to initialize UniEval DialogEvaluator: {e}") from e

    def _extract_data(self, data_item: Dict) -> Dict:
        """Extract required information from data item"""
        # Extract instruction, input, output
        instruction = data_item["instruction"]
        input_text = data_item.get("input", "")
        output = data_item["output"]
        
        # For dialogue, source is the dialogue history (instruction + input)
        # Format requirement: use \n to separate different turns, end with \n\n
        source_content = f"{instruction}\n{input_text}".strip() if input_text else instruction
        source = f"{source_content}\n\n"
        

                
        # context is required and must not be empty
        context = data_item["context"]
        if not context or context.strip() == "":
            raise ValueError("context is required and must not be empty")
    ...
    def score_item(self, data_item: Dict) -> Dict[str, float]:
        ...
        full_text = extracted["source"] + extracted["output"] + extracted["context"]
    ...
    def evaluate(self, dataset: str) -> List[Dict]:
        ...
        for i in tqdm(range(0, len(all_data), batch_size), desc="Evaluating batches"):
            ...
            # Consistency check: ensure the number of returned scores matches the batch size
            if len(eval_scores) != len(batch_data):
                raise RuntimeError(
                    f"Batch evaluation inconsistency: expected {len(batch_data)} scores, "
                    f"but got {len(eval_scores)} scores at batch starting index {i}"
                )
            ...
            for j, score_dict in enumerate(eval_scores):
                ...
                for dim, score in score_dict.items():
                    result[f"UniEval_Dialog_{dim.capitalize()}"] = score
